...
{	
	...
	{		
		...
		{				
			...
			{
				result._outputdirectory = System.IO.Path.GetDirectoryName (Path) +"/"+ result._outputdirectory;
			}
			...
			{							
				switch (node.Name.ToLower ())
				{
					case "project":
					{						
						result._projects.Add (Project.Parse (node.ChildNodes, node.Attributes["name"].Value, Path));
